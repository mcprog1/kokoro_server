...
public class ListaPrecioFacadeREST extends AbstractFacade<ListaPrecio> {
    ...
    @Path("{id}")
    @Consumes({MediaType.APPLICATION_JSON})
    public void edit(@PathParam("id") Long id, ListaPrecio entity) {
        super.edit(entity);
    }
    ...
    public ListaPrecio find(@PathParam("id") Long id) {
    ...
    @Path("{from}/{to}")
    @Produces({MediaType.APPLICATION_JSON})
    public List<ListaPrecio> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
